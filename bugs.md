...
- **Факт (reopened):** 1) "Онлайн-урок (живое занятие)" обрезан сверху — drawer не показывает первый элемент полностью. 2) Нет отступов/разделителей между секциями — всё сливается. 3) "Тест: Окружающий мир (1 класс)" — префикс "Тест:" лишний для ребёнка. 4) CAPS убран — ОК, chevrons убраны — ОК
- **Скриншот:** скриншот пользователя (drawer Окружающий мир)
- **Статус:** reopened — частично исправлен (CAPS/chevrons), но обрезка и отступы не сделаны
